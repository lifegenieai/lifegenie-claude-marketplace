...
### Step 4: Commit and Create PR (Full Update Mode Only)

**Skip this step if `--analyze-only` was specified.**

After the doc-updater completes, check if any documentation files were changed:

```bash
git status --porcelain docs/ CLAUDE.md
```

**If no changes detected**, skip to Step 5 and report "No documentation updates needed."

**If changes exist**, execute the full branch → commit → PR → reset workflow:

1. **Ensure clean starting point** — stash any non-doc changes if present:
...
3. **Stage only documentation files** (never stage `.claude/doc-sync/` working files):
...
Updated by /doc-sync:update-docs.
...
gh pr create \
  --title "docs: sync documentation ($(date +%Y-%m-%d))" \
  --body "## Summary

Documentation updates generated by \`/doc-sync:update-docs\`.

<insert list of files updated from doc-updater report>

🤖 Generated with [Claude Code](https://claude.com/claude-code)"
```

6. **Return to master and clean up** (squash-merge safe):

```bash
git checkout master
git reset --hard origin/master
git branch -D docs/sync-$(date +%Y-%m-%d)
```

**Branch name collision:** If `docs/sync-<date>` already exists (ran earlier today), append a
suffix: `docs/sync-<date>-2`, `docs/sync-<date>-3`, etc.
...
3. If a PR was created, display the PR URL
4. Show state file status:
...
5. Suggest running `/update-docs` again after more changes accumulate
