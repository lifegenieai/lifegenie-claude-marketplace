 ...
 *
 * Original: https://github.com/kingbootoshi/nano-banana-2-skill
 * Author: Bootoshi (https://github.com/kingbootoshi)
 *
 * Usage:
...
async function removeBackground(inputPath: string): Promise<string> {
  // Cross-platform: use dirname() instead of manual "/" splitting
  ...
  try {
    ...
    if (msg.includes("Failed to run ffmpeg") || msg.includes("ENOENT")) {
      ...
      console.error(`  Install it: brew install ffmpeg (macOS) or winget install ffmpeg (Windows)`);
      throw new Error("FFmpeg is required for transparent mode. Install: brew install ffmpeg (macOS) or winget install ffmpeg (Windows)");
    }
